#!/usr/bin/env python3
"""Build LibreKick M2.26: qualify successor-only dynamic FreeMem merge."""
from pathlib import Path
import struct, sys
import make_m2_25_rom as p
import make_m2_17_rom as m

MARKER=b'LIBREKICK-M2.26\0EXEC-DYNAMIC-SUCCESSOR-MERGE\0'
IDENT=b'exec.library\0LibreKick M2.26 dynamic successor-only FreeMem merge slice 40.26\0'
PROBE_OFF=0x2800
PROBE_END=0x2A00
...
def build():
    ...
    # Chain after M2.25. Qualify the successor-coalescing path independently:
    # freeing A2 next to the existing tail must merge forward while live A1
    # prevents a predecessor merge.
    prev=image[p.PROBE_OFF:p.PROBE_END]
    sig=bytes.fromhex('33FC00F000DFF1806000')
    gp=prev.rfind(sig)
    if gp<0: raise ValueError('M2.25 final success gate not found')
    ...
    # Tail is $9320/$CE0. Free A2 while A1 remains live. The new $100 chunk
    # is adjacent only to the successor tail, so it must merge forward into
    # one $DE0 chunk rooted at $9220.
    tail=m.APAY+0x300
    free(m.APAY+0x200,0x100)
    merged=m.APAY+0x200
    merged_bytes=m.AFREE-0x200
    fails += [
        m.cmpabs(c,merged,m.ABASE+m.MH_FIRST),
        m.cmpabs(c,0,merged),
        m.cmpabs(c,merged_bytes,merged+4),
        m.cmpabs(c,merged_bytes,m.ABASE+m.MH_FREE),
    ]

    # Free A1; this again merges with the successor while A0 stays live.
    free(m.APAY+0x100,0x100)
    merged2=m.APAY+0x100
    merged2_bytes=m.AFREE-0x100
    fails += [
        m.cmpabs(c,merged2,m.ABASE+m.MH_FIRST),
        m.cmpabs(c,0,merged2),
        m.cmpabs(c,merged2_bytes,merged2+4),
        m.cmpabs(c,merged2_bytes,m.ABASE+m.MH_FREE),
    ]

    # Free A0 to restore a single complete chunk, then restore static FAST.
    free(m.APAY,0x100)
    fails += [
    ...
    m.patch(c,ok,idle)
    if test_abs+len(c)>PROBE_END: raise ValueError('M2.26 runtime probe exceeds dedicated ROM window')
    if any(b != 0xff for b in image[test_abs:test_abs+len(c)]):
        raise ValueError('M2.26 runtime probe window is not unused')
...
if __name__=='__main__':
    if len(sys.argv)!=2: raise SystemExit('usage: make_m2_26_rom.py OUTPUT')
    out=Path(sys.argv[1]); data=build(); out.write_bytes(data)
    print(f'M2.26 ROM built: {out} ({len(data)} bytes)')
