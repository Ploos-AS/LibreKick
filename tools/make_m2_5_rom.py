...
def enqueue_code():
    q = bytearray()
    q += bytes.fromhex("2F002F012F082F092F0A2F0B")  # save d0/d1/a0/a1/a2/a3
    q += bytes.fromhex("7200122900094881")          # signed new priority -> d1
    q += bytes.fromhex("267C00000000")              # a3=pred=NULL
    q += bytes.fromhex("2450")                      # a2=list->head
    loop = len(q)
    q += bytes.fromhex("4A92")                      # tail sentinel? (succ == NULL)
    at_insert_1 = branch(q, 0x6700)                  # beq insert
    q += bytes.fromhex("7000102A00094880")          # signed current priority -> d0
    q += bytes.fromhex("B041")                      # cmp.w d1,d0
    at_insert_2 = branch(q, 0x6D00)                  # blt insert if current < new
    q += bytes.fromhex("264A2452")                  # pred=current; current=current->succ
    again = branch(q, 0x6000)
    insert = len(q)
    q += bytes.fromhex("244B")                      # a2=pred
    q += bytes.fromhex("4EAEFF16")                  # Insert(list,node,pred), -234(a6)
...
def findname_code():
    q = bytearray()
    q += bytes.fromhex("2F082F092F0A2F0B")          # save a0/a1/a2/a3
    q += bytes.fromhex("2450")                      # candidate = start->succ/head
    loop = len(q)
    q += bytes.fromhex("4A92")                      # tail sentinel has succ==NULL
    notfound_1 = branch(q, 0x6700)
    q += bytes.fromhex("206A000A")                  # a0=candidate->ln_Name
    q += bytes.fromhex("20084A80")                  # NULL name?
    next_1 = branch(q, 0x6700)
    q += bytes.fromhex("2649")                      # a3=search name
    chars = len(q)
    q += bytes.fromhex("1018B01B")                  # d0=*nodeName++; cmp.b *search++,d0
    next_2 = branch(q, 0x6600)
    q += bytes.fromhex("4A00")                      # terminating NUL?
    more = branch(q, 0x6600)
    q += bytes.fromhex("200A")                      # found: d0=candidate
    done_branch = branch(q, 0x6000)
    nxt = len(q)
    q += bytes.fromhex("2452")                      # candidate=candidate->succ
    loop_branch = branch(q, 0x6000)
    notfound = len(q)
    q += bytes.fromhex("7000")                      # d0=NULL
    done = len(q)
    q += bytes.fromhex("265F245F225F205F4A804E75") # restore; set Z from result; rts
...
def build():
    ...
    # struct Library-compatible header, public vector space through FindName.
    c += ml(0, EXEC_BASE + 0)
    c += ml(0, EXEC_BASE + 4)
    c += mw(0x0900, EXEC_BASE + 8)
    c += ml(IDSTRING_ADDR, EXEC_BASE + 10)
    c += mw(0, EXEC_BASE + 14)
    c += mw(276, EXEC_BASE + 16)
    ...
    for offset, target in FUNCS.items():
        c += vector(target, EXEC_BASE - offset)

    # Canonical empty List and full Nodes (succ,pred,type/pri,name).
    ...
    c += mw(0x0f00, COLOR00)                       # red: probe running
    c += b"\x4d\xf9" + struct.pack(">I", EXEC_BASE)

    # Enqueue priorities 10,5,10. Equal-priority FIFO => node1,node3,node2.
    for node in (NODE1, NODE2, NODE3):
        c += b"\x41\xf9" + struct.pack(">I", LIST_ADDR)
        c += b"\x43\xf9" + struct.pack(">I", node)
        c += bytes.fromhex("4EAEFEF2")              # Enqueue -270
    ...
    c += mw(0x0ff0, COLOR00)                       # yellow: Enqueue semantics pass

    def call_find(start, name):
        nonlocal c
        c += b"\x41\xf9" + struct.pack(">I", start)
        c += b"\x43\xf9" + struct.pack(">I", name)
        c += bytes.fromhex("4EAEFEEC")             # FindName -276
    ...
    c += mw(0x00f0, COLOR00)                       # green: M2.5 PASS
    ...
    # Retain the already-qualified M2.4 primitive implementations.
    routines = {
        ...
        0x0940: bytes.fromhex("2F012F082F0920280008224022290004670E21410008224141E80004228860027000225F205F221F4E75"),
        0x0A00: enqueue_code(),
        0x0A80: findname_code(),
    }
    ...
    image[ALPHA_OFF:ALPHA_OFF+6] = b"alpha\0"
    image[BETA_OFF:BETA_OFF+5] = b"beta\0"
