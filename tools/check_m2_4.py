...
FUNCS = {
    234: (0x0800, bytes.fromhex(
        "2F002F082F092F0A4A8A671220122280234A000420402149000424896010"
...
# Correct OVL handoff, Library V40.4 header, and negative space through RemTail.
...
# Exact public LVO slots/targets and implementation bodies.
...
# Bootstrap semantic probe calls every newly added primitive and both Insert modes.
...
# Routine regions must remain disjoint and below metadata.
...
print(f"M2.4 check PASS: {p} ({len(data)} bytes)")
print("Exec Insert/AddHead/AddTail/Remove/RemHead/RemTail; multi-node semantic probe; checksum=0xffffffff")
