...
BBASE=0xA000; BSIZE=0x800; BFREE=BSIZE-32
...
def build():
 ...
 patch(c,good,idle)
 if 8+len(c)>0x0B00: raise ValueError('bootstrap overlaps M2.15 routines')
 image[8:8+len(c)]=c
 ...
 marker=b'LIBREKICK-M2.15\0EXEC-LINKED-DYNAMIC-MEMLIST\0'; ident=b'exec.library\0LibreKick M2.15 linked dynamic MemList slice 40.15\0'; name_a=b'M2.15 dynamic FAST A\0'; name_b=b'M2.15 dynamic CHIP B\0'
 image[MARKER_OFF:MARKER_OFF+len(marker)]=marker; image[IDENT_OFF:IDENT_OFF+len(ident)]=ident; image[NAME_A_OFF:NAME_A_OFF+len(name_a)]=name_a; image[NAME_B_OFF:NAME_B_OFF+len(name_b)]=name_b
