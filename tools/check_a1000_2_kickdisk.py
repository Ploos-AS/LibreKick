#!/usr/bin/env python3
"""Static validation for LibreKick A1000.7 shared-Exec WCS payload/disk."""
from hashlib import sha256
from pathlib import Path
import struct,sys
from librekick_exec_abi import EXEC_BASE,LIB_VERSION_OFF,LIB_REVISION_OFF,LIB_IDSTRING_OFF,EXEC_VERSION
from librekick_exec_runtime import SYSBASE_ADDR,THIS_TASK_OFF,get_sysbase_code,get_current_task_code
import make_a1000_2_kickdisk as builder

def main():
    root=Path(sys.argv[1] if len(sys.argv)>1 else "build/a1000")
    pp=root/"librekick-a1000.7-wcs.bin"; dp=root/"librekick-a1000.7-kickdisk.adf"
    ...
    gsb=get_sysbase_code(); gct=get_current_task_code()
    ...
    print(f"A1000.7 static check PASS: {dp} ({len(disk)} bytes)")
    print(f"WCS payload={len(payload)} bytes load=${load_addr:08x} entry=${entry_pc:08x}")
    print(f"shared_exec=ExecBase@${EXEC_BASE:08x} version={EXEC_VERSION}.{builder.REVISION} primitives=GetSysBase,GetCurrentTask")
    print(f"payload_sha256={sha256(payload).hexdigest()}")
    print("format=LibreKick-private bootstrap container v2; public FindTask/full Exec compatibility not claimed")
    return 0
