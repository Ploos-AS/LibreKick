#!/usr/bin/env python3
"""Build deterministic LibreKick A1000.8 WCS shared-Exec payload/disk."""
from __future__ import annotations
from hashlib import sha256
from pathlib import Path
import struct, sys
from librekick_exec_abi import EXEC_BASE, LIB_VERSION_OFF, LIB_REVISION_OFF, LIB_IDSTRING_OFF, EXEC_VERSION
from librekick_exec_runtime import SYSBASE_ADDR, THIS_TASK_OFF, get_sysbase_code, get_current_task_code, set_current_task_code, jsr_absolute
...
FORMAT_VERSION=2
PAYLOAD_MARKER=b"LIBREKICK-A1000.8\0WCS-SHARED-EXEC-TASK-SET-RUNTIME\0"
DISK_MARKER=b"LIBREKICK-A1000.8\0KICKDISK-CONTAINER\0"
IDENT=b"exec.library\0LibreKick A1000.8 shared Exec task set runtime 40.8\0"
IDENT_OFF=0x180
...
def runtime_code():
    ...
    code+=jsr_absolute(WCS_BASE+SETCURRENTTASK_OFF)
...
def build_payload():
    ...
    code=runtime_code(); gsb=get_sysbase_code(); gct=get_current_task_code(); sct=set_current_task_code()
    if 8+len(code)>0x100: raise ValueError("A1000.8 runtime overlaps payload marker")
...
def main():
    ...
    pp=out/"librekick-a1000.8-wcs.bin"; dp=out/"librekick-a1000.8-kickdisk.adf"
    payload=build_payload(); disk=build_disk(payload); pp.write_bytes(payload); dp.write_bytes(disk)
    print(f"A1000.8 WCS payload built: {pp} ({len(payload)} bytes)")
    print(f"A1000.8 kickdisk built: {dp} ({len(disk)} bytes)")
    print(f"exec_base=${EXEC_BASE:08x} version={EXEC_VERSION}.{REVISION} shared_primitives=GetSysBase,GetCurrentTask,SetCurrentTask")
