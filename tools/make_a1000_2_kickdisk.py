#!/usr/bin/env python3
"""Build deterministic LibreKick A1000.10 WCS task-stack handoff payload/disk."""
...
FORMAT_VERSION=2
PAYLOAD_MARKER=b"LIBREKICK-A1000.10\0WCS-SHARED-EXEC-TASK-STACK-HANDOFF\0"
DISK_MARKER=b"LIBREKICK-A1000.10\0KICKDISK-CONTAINER\0"
IDENT=b"exec.library\0LibreKick A1000.10 private task stack handoff 40.10\0"
MARKER_OFF=0x300
IDENT_OFF=0x380
GETSYSBASE_OFF=0x400
GETCURRENTTASK_OFF=0x420
SETCURRENTTASK_OFF=0x440
SWAPCURRENTTASK_OFF=0x460
HANDOFFTASKSTACK_OFF=0x480
RESUME_OFF=0x4C0
COLOR00=0x00DFF180
REVISION=10
...
def resume_code():
    # First handoff returns here on TASK_REPLACEMENT's prepared stack with
    # D0=TASK_SENTINEL. Calling the same helper again saves the replacement
    # stack, restores the original task stack and RTSes to the original caller.
    return jsr_absolute(WCS_BASE+HANDOFFTASKSTACK_OFF)+bytes.fromhex("60fe")
...
def runtime_code():
    ...
    code+=_ml(0,TASK_SENTINEL+TC_SPREG_OFF)
    code+=_ml(NEW_STACK_RETURN,TASK_REPLACEMENT+TC_SPREG_OFF)
    code+=_ml(WCS_BASE+RESUME_OFF,NEW_STACK_RETURN)
    ...
    code+=jsr_absolute(WCS_BASE+GETSYSBASE_OFF); code+=_cmp_d0_l(EXEC_BASE); failures.append(_branch(code,0x6600))
    code+=jsr_absolute(WCS_BASE+GETCURRENTTASK_OFF); code+=_cmp_d0_l(TASK_SENTINEL); failures.append(_branch(code,0x6600))
    code+=_move_d0_l(TASK_REPLACEMENT)
    code+=jsr_absolute(WCS_BASE+HANDOFFTASKSTACK_OFF)
    # The first helper RTS goes to RESUME_OFF on the replacement stack. The
    # resume trampoline invokes the helper again and returns here on old A7.
    code+=_cmp_d0_l(TASK_REPLACEMENT); failures.append(_branch(code,0x6600))
    code+=_cmp_l(TASK_SENTINEL,EXEC_BASE+THIS_TASK_OFF); failures.append(_branch(code,0x6600))
    code+=_cmp_l(NEW_STACK_RETURN,TASK_REPLACEMENT+TC_SPREG_OFF); failures.append(_branch(code,0x6600))
    code+=_cmp_l(0,TASK_SENTINEL+TC_SPREG_OFF); failures.append(_branch(code,0x6700))
    code+=jsr_absolute(WCS_BASE+GETCURRENTTASK_OFF); code+=_cmp_d0_l(TASK_SENTINEL); failures.append(_branch(code,0x6600))
...
def build_payload():
    ...
    code=runtime_code(); gsb=get_sysbase_code(); gct=get_current_task_code(); sct=set_current_task_code(); swp=swap_current_task_code(); hts=handoff_task_stack_code(); resume=resume_code()
    if 8+len(code)>MARKER_OFF: raise ValueError("A1000.10 runtime overlaps payload marker")
    ...
    payload[SETCURRENTTASK_OFF:SETCURRENTTASK_OFF+len(sct)]=sct
    payload[SWAPCURRENTTASK_OFF:SWAPCURRENTTASK_OFF+len(swp)]=swp
    payload[HANDOFFTASKSTACK_OFF:HANDOFFTASKSTACK_OFF+len(hts)]=hts
    payload[RESUME_OFF:RESUME_OFF+len(resume)]=resume
...
def main():
    ...
    pp=out/"librekick-a1000.10-wcs.bin"; dp=out/"librekick-a1000.10-kickdisk.adf"
    payload=build_payload(); disk=build_disk(payload); pp.write_bytes(payload); dp.write_bytes(disk)
    print(f"A1000.10 WCS payload built: {pp} ({len(payload)} bytes)")
    print(f"A1000.10 kickdisk built: {dp} ({len(disk)} bytes)")
    print(f"exec_base=${EXEC_BASE:08x} version={EXEC_VERSION}.{REVISION} shared_primitives=GetSysBase,GetCurrentTask,SetCurrentTask,SwapCurrentTask,HandoffTaskStack")
