PYTHON ?= python3
BUILD_DIR := build
ROM := $(BUILD_DIR)/librekick-m2_47.rom
...
A1000_WCS := $(BUILD_DIR)/a1000/librekick-a1000.12-wcs.bin
A1000_KICKDISK := $(BUILD_DIR)/a1000/librekick-a1000.12-kickdisk.adf
A500_PROFILE_ROM := $(BUILD_DIR)/a500/librekick-m2_47-a500.rom
...
$(ROM): tools/make_m2_47_rom.py
	mkdir -p $(BUILD_DIR)
	$(PYTHON) tools/make_m2_47_rom.py $@
check: $(ROM)
	$(PYTHON) tools/check_m2_47.py $(ROM)
a1000-bootstrap:
...
a1000-12-kickdisk:
	mkdir -p $(BUILD_DIR)/a1000
	$(PYTHON) tools/make_a1000_12_kickdisk.py $(BUILD_DIR)/a1000
check-a1000-12-kickdisk: a1000-12-kickdisk
	$(PYTHON) tools/check_a1000_12_kickdisk.py $(BUILD_DIR)/a1000
a1000-loader:
	mkdir -p $(BUILD_DIR)/a1000
	$(PYTHON) tools/build_a1000_3_bootstrap.py $(A1000_LOADER_ROM)
check-a1000-loader: a1000-loader check-a1000-12-kickdisk
	$(PYTHON) tools/check_a1000_3_bootstrap.py $(A1000_LOADER_ROM)
qualify-a1000-loader: check-a1000-loader
	fs-uae configs/fs-uae/a1000-bootstrap-current.fs-uae
qualify-m2_47: check
	fs-uae configs/fs-uae/a500-m2_47.fs-uae
m2_47:
	mkdir -p $(BUILD_DIR)
	$(PYTHON) tools/make_m2_47_rom.py $(BUILD_DIR)/librekick-m2_47.rom
	$(PYTHON) tools/check_m2_47.py $(BUILD_DIR)/librekick-m2_47.rom
qualify-m2_48: m2_48
	fs-uae configs/fs-uae/a500-m2_48.fs-uae
m2_48:
	mkdir -p $(BUILD_DIR)
